<template>
  <div>
    ...
    <button
      ...
      :class="{ 'uk-button-primary': !isPushEnabled, 'uk-button-danger': isPushEnabled }">
      ウェブプッシュ通知 {{ isPushEnabled ? '無効化' : '有効化' }}
    </button>
  </div>
</template>
  ...
  methods: {
    ...
    subscribe() {
      navigator.serviceWorker.ready.then(registration => {
        ...
        const vapidPublicKey = document.head.querySelector(
          'meta[name="vapidPublicKey"]'
        ).content;
    ...
    updateSubscription(subscription) {
      ...
      axios.post("/push/subscriptions", data).then(() => {
        this.loading = false;
      });
    },
    ...
    deleteSubscription(subscription) {
      ...
      axios
        .post("/push/subscriptions/delete", { endpoint: subscription.endpoint })
        .then(() => {
          this.loading = false;
        });
    },
